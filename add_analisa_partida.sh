...
BRANCH="feature/analisa-partida"
COMMIT_MSG="feat(api): rota /api/analisa-partida com resumo via OpenAI"

# Detecta a pasta de API (cria se não existir)
API_DIR=""
...
TARGET="$API_DIR/analisa-partida.ts"
echo "→ Criando $TARGET"
...
echo ""
echo "✅ Rota criada e enviada para preview ($BRANCH)."
echo "• Local (rode npm run dev):    http://localhost:3000/api/analisa-partida"
echo "• Preview Vercel: abra o último deployment da branch $BRANCH e acesse /api/analisa-partida"
echo ""
echo "Exemplo de teste (curl):"
cat <<'CURL'
curl -sS -X POST "http://localhost:3000/api/analisa-partida" \
