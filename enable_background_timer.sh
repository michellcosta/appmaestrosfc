...
STORE="src/store/matchStore.ts"
PAGE="src/pages/Match.tsx"

[ -f "$STORE" ] && cp -f "$STORE" "$STORE.bak" || true
...
export const useMatchStore = create<MatchStore>()(
  persist(
    (set, get) => ({
      ...
      recomputeScores: () => {
        const base: Score = { Preto:0, Verede:0, Cinza:0, Vermelho:0 } as any
...
# corrige typo de "Verede" se gerado por engano
sed -i 's/Verede/Verde/g' "$STORE" || true

# Atualiza Match.tsx para usar elapsed derivado e remover tick()
[ -f "$PAGE" ] && cp -f "$PAGE" "$PAGE.bak" || true
# Aqui apenas garantimos que ele não dependa de tick() e sim do getElapsedSec()
# Não sobrescrevemos tudo: instruções para você ajustar caso precise
echo "✅ Store atualizada. Agora ajuste o Match.tsx para usar elapsed derivado:"
echo "   - Importe:  import { useMatchStore } from '@/store/matchStore'"
echo "   - Substitua qualquer 'elapsed' do estado local por:"
echo "       const elapsed = useMatchStore(s => s.getElapsedSec())"
echo "   - Remova o useEffect que chamava tick() a cada 1s."
echo "   - Os botões Iniciar/Pausar/Recomeçar continuam chamando start/pause/reset do store."
