...
echo "��� Backup..."
cp -f "$FILE" "$FILE.bak.end-round"

echo "��� Reescrevendo Match.tsx com modal de Encerrar rodada..."
cat > "$FILE" <<'TSX'
...
const Match: React.FC = () => {
  ...
  const [nextTeamChoice, setNextTeamChoice] = useState<TeamColor | '_auto'>('auto_')
  ...
  const openEnd = () => {
    setNextTeamChoice('_auto_')
    setEndOpen(true)
  }
  const confirmEnd = () => {
    const choice = (nextTeamChoice === '_auto_') ? null : nextTeamChoice
    endRoundChooseNext(choice)
    setEndOpen(false)
  }
      ...
      <Dialog open={endOpen} onOpenChange={setEndOpen}>
        <DialogContent>
          ...
          <div className="grid gap-2">
            ...
            <Select value={nextTeamChoice} onValueChange={(v)=>setNextTeamChoice(v as any)}>
              <SelectTrigger><SelectValue placeholder="Selecione o próximo" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="_auto_">Automático</SelectItem>
                {candidatos.map((t)=>(
...
echo "✅ Atualizado. Commit & push..."
git add "$FILE" "$FILE.bak.end-round"
git commit -m "feat(match): modal de Encerrar rodada com escolha do próximo time; integra endRoundChooseNext"
git push origin feature/supabase-setup

echo "��� Pronto! Recarregue o preview e teste o botão Encerrar."
