...
import { type PlayerStats } from '@/utils/figurinhas';
import { calculateLevel, type PlayerRewards } from '@/utils/rewards';
import { validateImageFile, validatePlayerInfo, validateStatsData } from '@/utils/userValidation';
...
export default function PerfilPage() {
  ...
  // Convex: buscar profile real e mutation para salvar
  const profile = useQuery(api.profiles.getProfileByEmail,
    user?.email ? { email: user.email } : "skip"
  );
  const { updateProfileFields } = usePlayersConvex();
  ...
  // Carregar dados do profile do Convex
  useEffect(() => {
    if (profile) {
      setPlayerPosition(profile.position ?? 'Meia');
      setShirtSize(profile.shirt_size ?? 'G');
    }
  }, [profile]);
                  ...
                  <button
                    onClick={async () => {
                      if (isEditingInfo) {
                        ...
                        // Dados válidos, salvar no Convex
                        try {
                          if (user?.email) {
                            ...
                            console.log('✅ Informações salvas no Convex com sucesso!');
                          }
                        } catch (error) {
                    ...
                    className={`flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${isEditingInfo
          ...
          <CardContent>
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
              ...
              {achievements.map((achievement) => (
                <div
                  key={achievement.id}
                  className={`relative text-center p-3 rounded-lg border-2 shadow-md transition-all duration-300 ${achievement.unlocked
                ...
                >
                  ...
                  <div className={`text-xs font-bold drop-shadow-md ${achievement.unlocked ? 'text-white' : 'text-gray-500'
                    }`}>
                    {achievement.name}
                  </div>
                  <div className={`text-xs ${achievement.unlocked
                    ? 'text-gray-100'
                    : 'text-gray-500'
                    }`}>
