...
export interface Player {
    ...
    avatar_url?: string; // Compatibilidade
}
...
export function usePlayersConvex() {
    try {
        ...
        const syncProfileToPlayerMutation = useMutation(api.managedPlayers.syncProfileToPlayer);
        const updateProfileFieldsMutation = useMutation(api.managedPlayers.updateProfileFields);
        ...
        const syncProfileToPlayer = async (email: string, name: string, role?: 'owner' | 'admin' | 'aux' | 'player'): Promise<Id<"players">> => {
            try {
                const playerId = await syncProfileToPlayerMutation({ email, name, role });
                return playerId;
            } catch (error) {
                console.error('Erro ao sincronizar profile com player:', error);
                throw error;
            }
        };

        const updateProfileFields = async (email: string, position?: string, shirt_size?: string, phone?: string): Promise<Id<"profiles">> => {
            try {
                return await updateProfileFieldsMutation({ email, position, shirt_size, phone });
            } catch (error) {
                console.error('Erro ao atualizar campos do profile:', error);
                throw error;
            }
        };
        ...
        console.error('Erro no usePlayersConvex:', error);
        return {
            // Data
            players: [],
            approvedPlayers: [],
            ...
            // Actions
            createPlayer: async () => { throw new Error('Convex não disponível'); },
            updatePlayer: async () => { throw new Error('Convex não disponível'); },
            removePlayer: async () => { throw new Error('Convex não disponível'); },
            toggleApproval: async () => { throw new Error('Convex não disponível'); },
            syncProfileToPlayer: async () => { throw new Error('Convex não disponível'); },
            updateProfileFields: async () => { throw new Error('Convex não disponível'); },
